...
/// struct for typed errors of method [`api_configuration_set_polling_rate_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiConfigurationSetPollingRatePostError {
    Status401(crate::models::ProblemDetails),
    Status403(crate::models::ProblemDetails),
    UnknownValue(serde_json::Value),
}
...
pub async fn api_configuration_set_polling_rate_post(configuration: &configuration::Configuration, polling_rate: Option<i32>, device_id: Option<&str>) -> Result<(), Error<ApiConfigurationSetPollingRatePostError>> {
    ...
    let local_var_uri_str = format!("{}/api/Configuration/SetPollingRate", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ApiConfigurationSetPollingRatePostError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
